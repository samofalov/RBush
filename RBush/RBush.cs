...
namespace RBush
{
	public partial class RBush<T> : ISpatialDatabase<T>, ISpatialIndex<T> where T : ISpatialData
	{
		...
		public IReadOnlyList<T> Search(in Envelope boundingBox) =>
			DoPathSearch(boundingBox).Select(x => (T)x.Peek()).ToList();

		public IReadOnlyList<T> Search2(in Envelope boundingBox) =>
			DoSearchDirect(boundingBox);

		public IReadOnlyList<T> Search3(in Envelope boundingBox) =>
